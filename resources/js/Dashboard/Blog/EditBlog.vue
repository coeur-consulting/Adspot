<template>
  ...
  <form @submit.prevent="submit">
    <legend class="text-center font-bold mb-4">Update Blog</legend>
 ...
 <BreezeCheckbox id="status" class="mr-2" v-model="form.status"  :checked="form.status?true:false"/>
...
export default {
  ...
  methods: {
    ...
    update() {
      axios
        .put(this.route("blogs.update", this.$props.blog.id), this.form)
        .then((res) => {
          if (res.status === 200) {
